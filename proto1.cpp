...
struct Game {
  ...
  Game() {
    ...
    // Create Player
    G::entity.push_back(make_shared<Player>(
        Vec2(4.f * G::screenWidth / 5.f, G::screenHeight - 10.f)));
    // Load level
  ...
  static void removeDestroyedEntities(vector<shared_ptr<Entity>>& vec) {
    vec.erase(
        remove_if(begin(vec), end(vec),
                  [](const shared_ptr<Entity>& e) { return e->destroyed; }),
        end(vec));
  }
  ...
  void inputPhase() {
    ...
    if (Keyboard::isKeyPressed(Keyboard::Key::Space)) {
      // player shoot
      shared_ptr<Player> p_ptr =
          dynamic_pointer_cast<Player>(Entity::withId(0));
      if (p_ptr->mCanShoot) {
