...
void Entity::update(FrameTime ftStep) {
  /* check for collision */ /* update pos */
}
...
// ----------------------------------------
void Entity::setPos(Vec2 pos) { this->move(pos - this->pos); }
//-----------------------------------------
...
void Entity::eraseDeadVoxel() {
  ...
  vox.erase(remove_if(begin(vox), end(vox),
                      [](const Voxel& v) { return *v.health <= 0; }),
            end(vox));
...
shared_ptr<Entity> Entity::withId(size_t id) {
  return *find_if(begin(G::entity), end(G::entity),
                  [&id](const shared_ptr<Entity>& e) { return e->id == id; });
}
...
// Bullet types
Bullet::Bullet(Vec2 vv) : vel{vv} { o_type = EntityType::Bullet; }

void Bullet::update(FrameTime ftStep) {
  move(ftStep * vel);
  move(ftStep * dvel);
  // mark for destruction if go off screen
  if (pos.x < 0 || pos.x > G::screenWidth || pos.y < 0 ||
      pos.y > G::screenHeight) {
    destroyed = true;
  }
}
...
void Bullet::collideWith(Entity& e, unsigned int ivox, Vec2 voxPos) {
  ...
  switch (e.o_type) {
    case EntityType::Wall1:
      // need to move whole entity so other pixels don't hit it to get an even
      // bounce
...
// Enemy types
Enemy::Enemy() : currPathPoint{0} { o_type = EntityType::Enemy; }

void Enemy::update(FrameTime ftStep) {
  auto _pos = getPos();
  auto pathPoint = path[currPathPoint];
  float dist2goal =
      sqrt(pow(_pos.x - pathPoint.x, 2) + pow(_pos.y - pathPoint.y, 2));
  if (dist2goal < 3.f) {
