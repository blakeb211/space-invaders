...
void Builder::build_level(unsigned int &levelId) {
  ...
  while (in_file.good()) {
    ...
    if (line.find('|') != string::npos) {
      ...
      shared_ptr<Enemy> e_ptr =
          dynamic_pointer_cast<Enemy>(Entity::withId(currEntityId));
      // read in the path
      ...
      while (ss.good()) {
        ...
        e_ptr->path.push_back(Vec2(x * G::screenWidth * kCoordsConv,
                                   y * G::screenHeight * kCoordsConv));
      }
    ...
    if (line.find('B') != string::npos || line.find('D') != string::npos) {
      ...
      while (ss.good()) {
        ...
        if (c == 'B') {
          ...
          G::entity.push_back(
              make_shared<Wall1>(Vec2(x_start, y_start), Vec2(x_end, y_end)));
...
void Builder::build_wall1(Vec2 start, Vec2 end, vector<Voxel> &vox) {
  ...
  while (calc_dist(start, end) > 1 * G::blockWidth) {
    // place voxel
    vox.emplace_back(start.x, start.y, Color(210, 55, 70, 255));
    start += unitVec * 1.f * G::blockWidth;
  }
